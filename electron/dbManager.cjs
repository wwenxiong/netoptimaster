...
const FIXED_HEADERS_4G_MAP = new Map();
 ...
 * @param {Object} raw 原始行数据对象
 * @param {string} networkType 网络类型
 * @returns {Object} 过滤后的行数据对象（使用固定表头中的标准列名作为key）
 */
function filterRowByFixedHeaders(raw, networkType) {
    // 目前只对 4G 类型做固定表头过滤
    if (networkType !== '4G') {
        return raw;
    }
...
function getUniqueCellId(raw) {
    ...
    const getValueIgnoreCaseAndSpace = (keys) => {
        // 先完全匹配
        for (const k of keys) {
...
function handleRequest(action, payload, sendProgress) {
    ...
    switch (action) {
        ...
        case 'GET_CELL_CHANGES': {
            ...
            const rowsLatest = db.prepare(`SELECT cellName, rawData FROM ${table} WHERE networkType = ? AND granularity = ? AND timestamp = ?`).all(networkType, granularity, latestDate);
            ...
            if (prevDate) {
                ...
                const rowsPrev = db.prepare(`SELECT cellName, rawData FROM ${table} WHERE networkType = ? AND granularity = ? AND timestamp = ?`).all(networkType, granularity, prevDate);
